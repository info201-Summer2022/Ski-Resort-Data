server <- function(input, output){
  ...
  output$scatterplot <- renderPlot({
    ...
    ggplot(filter_df, aes(x = Selling_price, y = Total_price)) + geom_point() + xlab("Selling price") + ylab("Total Price") + 
      labs(title = "Relationship between Selling price and Total pice") + geom_smooth(method=lm, color = "blue", se = FALSE) + theme(axis.title = element_text(size = 10), plot.title = element_text(size = 13, hjust = 0.5))  
  })
